...
{
    internal partial class PluginBase
    {
        #region " Fields "
        ...
        private static string sectionName = "Insert Extension";

        private static string keyName = "doCloseTag";

        private static bool doCloseTag = false;

        private static string sessionFilePath = @"C:\text.session";

        private static frmGoToLine frmGoToLine = null;
        ...
        internal static void CommandMenuInit()
        {
            ...
            SetCommand(
                9,
                "Close HTML/XML tag automatically",
                checkInsertHtmlCloseTag,
                new ShortcutKey(false, true, false, Keys.Q),
                doCloseTag);
        ...
        internal static void SetToolBarIcon()
        {
            ...
            Win32.SendMessage(
                nppData._nppHandle, NppMsg.NPPM_ADDTOOLBARICON, _funcItems.Items[idFrmGotToLine]._cmdID, pTbIcons);
        ...
        private static void helloFX()
        {
            hello();
            new Thread(callbackHelloFX).Start();
        }

        private static void callbackHelloFX()
        {
        ...
        private static void callbackWhatIsNpp(object data)
        {
            ...
            for (int i = 0; i < text2display.Length; i++)
            {
                ...
                Win32.SendMessage(
                    curScintilla,
                    SciMsg.SCI_GOTOPOS,
                    (int)Win32.SendMessage(curScintilla, SciMsg.SCI_GETLENGTH, 0, 0),
                    0);
            }
        }

        private static void insertCurrentFullPath()
        {
            insertCurrentPath(NppMsg.FULL_CURRENT_PATH);
        }

        private static void insertCurrentFileName()
        {
            insertCurrentPath(NppMsg.FILE_NAME);
        }

        private static void insertCurrentDirectory()
        {
            insertCurrentPath(NppMsg.CURRENT_DIRECTORY);
        }

        private static void insertCurrentPath(NppMsg which)
        {
            NppMsg msg = NppMsg.NPPM_GETFULLCURRENTPATH;
            if (which == NppMsg.FILE_NAME) msg = NppMsg.NPPM_GETFILENAME;
            else if (which == NppMsg.CURRENT_DIRECTORY) msg = NppMsg.NPPM_GETCURRENTDIRECTORY;
        ...
        private static void insertShortDateTime()
        {
            insertDateTime(false);
        }

        private static void insertLongDateTime()
        {
            insertDateTime(true);
        }

        private static void insertDateTime(bool longFormat)
        {
        ...
        private static void checkInsertHtmlCloseTag()
        {
            doCloseTag = !doCloseTag;

            int i = Win32.CheckMenuItem(
                Win32.GetMenu(nppData._nppHandle),
                _funcItems.Items[9]._cmdID,
                    ...
                    if (size >= 3)
                    {
                        using (Sci_TextRange tr = new Sci_TextRange(startPos, currentPos, bufCapacity))
                        {
                            ...
                            if (buf[size - 2] != '/')
                            {
                                ...
                                int pCur = size - 2;
                                for (; (pCur > 0) && (buf[pCur] != '<') && (buf[pCur] != '>');) pCur--;
        ...
        private static void getFileNamesDemo()
        {
            ...
            {
                if (Win32.SendMessage(nppData._nppHandle, NppMsg.NPPM_GETOPENFILENAMES, cStrArray.NativePointer, nbFile)
                    != IntPtr.Zero) foreach (string file in cStrArray.ManagedStringsUnicode) MessageBox.Show(file);
            }
        }

        private static void getSessionFileNamesDemo()
        {
            ...
            {
                MessageBox.Show(
                    "Please modify \"sessionFilePath\" in \"Demo.cs\" in order to point to a valid session file",
                    "Error");
                return;
            }
            ...
            using (ClikeStringArray cStrArray = new ClikeStringArray(nbFile, Win32.MAX_PATH))
            {
                if (Win32.SendMessage(
                    nppData._nppHandle, NppMsg.NPPM_GETSESSIONFILES, cStrArray.NativePointer, sessionFilePath)
                    != IntPtr.Zero) foreach (string file in cStrArray.ManagedStringsUnicode) MessageBox.Show(file);
            }
        }

        private static void saveCurrentSessionDemo()
        {
            string sessionPath =
                Marshal.PtrToStringUni(
                    Win32.SendMessage(nppData._nppHandle, NppMsg.NPPM_SAVECURRENTSESSION, 0, sessionFilePath));
            if (!string.IsNullOrEmpty(sessionPath)) MessageBox.Show(sessionPath, "Saved Session File :", MessageBoxButtons.OK);
        }

        private static void DockableDlgDemo()
        {
            ...
            if (frmGoToLine == null)
            {
                ...
                Win32.SendMessage(
                    nppData._nppHandle, NppMsg.NPPM_SETMENUITEMCHECK, _funcItems.Items[idFrmGotToLine]._cmdID, 1);
            }
            else
            {
                ...
                else
                {
                    ...
                    Win32.SendMessage(
                        nppData._nppHandle, NppMsg.NPPM_SETMENUITEMCHECK, _funcItems.Items[idFrmGotToLine]._cmdID, 0);
                }
            }
